...
module xrouter #(
  ...
  input mmap_t spaces[ASN],
  output port_idx_t idx
);

  `define addr_hit(space) (valid && addr >= space.BASE && addr <= space.END)
  always_comb begin
    idx = port_idx_t'(ASN);
...
module fadd (
  ...
  function automatic ffast_t check_fastpath();
    ...
    if (zero) begin
      if (both_zero) begin
        if (rm_i == RDN) begin
          res.result = (s1 == s2 ? op1_i : `fp_zero(single_i, 1'b1));
        end else begin
          res.result = (s1 == s2 ? op1_i : `fp_zero(single_i, 1'b0));
        end
      end else begin
  ...
  function automatic faligned_t alignment(funpack_t u1, funpack_t u2);
    ...
    if (s1 ^ s2) begin
      res.exp = exp;
      if (m1 >= m2) begin
        res.sign  = m1 == m2 ? (rm_i == RDN ? 1 : 0) : s1;
        res.manti = m1 - m2;
      end else begin
        res.sign  = s2;
        res.manti = m2 - m1;
  ...
  function automatic ffast_t check_fastpath();
    ...
    if (a12.INF | a12.ZERO | a12.NAN | a3.INF | a3.NAN) begin
      if (a12.NAN | a3.NAN) begin
        ...
        if (a3.ZERO) begin
          if (s12 != s3) begin
            res.result = `fp_zero(single_i, (rm_i == RDN ? 1'b1 : 1'b0));
          end
        end
      end
      res.flags.nv = nv;
