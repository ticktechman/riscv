...
module minisoc (
  ...
  always_comb begin
    case (if_inst[6:0])
      7'b0010011, 7'b0000011, 7'b1100111: imm = imm_i(if_inst);
      7'b0100011: imm = imm_s(if_inst);
      7'b1100011: imm = imm_b(if_inst);
      7'b1101111: imm = imm_j(if_inst);
      default: imm = 0;
    endcase
  end
  ...
  // Hazard
  // ============================================================
  assign stall = ex_valid && ex_ctrl.mem_read && (ex_rd != 0) && ((ex_rd == id_rs1) || (ex_rd == id_rs2));
  ...
  always_ff @(posedge clk) begin
    if (!rst) begin
      ...
      ex_ctrl <= id_ctrl;
      ex_pc <= id_pc;
      ex_rs1_val <= regfile[id_rs1];
      ex_rs2_val <= regfile[id_rs2];
      ex_rs1 <= id_rs1;
      ex_rs2 <= id_rs2;
      ex_rd <= id_rd;
      ex_imm <= id_imm;
  ...
  always_ff @(posedge clk) begin
    if (mem_valid && mem_ctrl.mem_write) begin
      `LOGI($sformatf("m[%02d]=%h", mem_alu_result[12:3], mem_wdata));
  ...
  always_ff @(posedge clk) begin
    if (wb_valid && wb_ctrl.reg_write && wb_rd != 0) begin
      `LOGI($sformatf("r[%02d]=%0d", wb_rd, wb_data));
